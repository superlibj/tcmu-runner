...
static inline struct log_rb *current_rb(void)
{
	struct log_rb *r;
...
static bool tcmu_log_initialize(void)
{
	...
	pthread_mutex_lock(&r->logbuf->lock);
	while (!r->logbuf->finish_initialize)
		pthread_cond_wait(&r->logbuf->cond, &r->logbuf->lock);
	pthread_mutex_unlock(&r->logbuf->lock);
	pthread_mutex_unlock(&g_mutex);
